...
export const getPublishedProductCatalog = cache(async () => {
...
export const getPublishedProductShowcaseCatalog = cache(async (): Promise<{
  products: ProductShowcaseProduct[];
  categories: string[];
}> => {
  const { products, categories } = await getPublishedProductCatalog();
...
export const getPublishedProductBySlug = cache(async (slug: string): Promise<ProductDetailResult | null> => {
  const supabase = await getSupabase();
