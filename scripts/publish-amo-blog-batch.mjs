...
import { amoBlogArticles, standardCallToAction } from "./amo-blog-batch-data.mjs";
import { articleSupplements } from "./amo-blog-batch-supplements.mjs";
import { articleSupplementExtensions } from "./amo-blog-batch-supplements-2.mjs";
import { articleSupplementFinalPass } from "./amo-blog-batch-supplements-3.mjs";
...
async function processHeroImage(article) {
  ...
  const sourcePath = path.join(heroSourceDir, `${article.slug}.png`);
...
function validateArticle(article) {
  const supplement = articleSupplements[article.slug] ?? "";
  const extension = articleSupplementExtensions[article.slug] ?? "";
  const finalPass = articleSupplementFinalPass[article.slug] ?? "";
  const content = [article.body, supplement, extension, finalPass, standardCallToAction]
    .filter(Boolean)
    .join("\n\n");
  const wordCount = countWords(content);
  const bodyBlocks = parseBlogContentInput(content);

  if (wordCount < 1500 || wordCount > 2500) {
    throw new Error(`${article.slug} has ${wordCount} words; expected 1500-2500.`);
  }

  if (bodyBlocks.length < 5) {
...
async function publishArticle(supabase, article) {
  const { content, bodyBlocks, wordCount, readTime } = validateArticle(article);
    ...
    .upsert(
      {
        ...
        tags: ["AMO", "magnetic levitation", article.category],
        status: "published",
        featured: false,
...
async function main() {
  const validations = amoBlogArticles.map((article) => ({
    title: article.title,
    slug: article.slug,
  ...
  for (const article of amoBlogArticles) {
